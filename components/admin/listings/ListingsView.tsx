...
import { PencilIcon, BriefcaseIcon, SearchIcon, SpinnerIcon } from "@/components/icons";
...
export default function ListingsView({ initialStatus = "all" }: { initialStatus?: ListingStatus | "all" }) {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    fetchBusinessListings().then((rows) => {
      if (cancelled) return;
      setListings(rows);
      setIsLoading(false);
    });
  ...
  async function handleSave(values: ListingFormValues) {
    ...
    try {
      const updated = await updateBusinessListing(editingListing.id, values);
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        >
          <SpinnerIcon className="size-4 animate-spin" /> Loading listings…
        </motion.div>
      ) : (
        <>
          ...
          {filtered.length === 0 ? (
            listings.length === 0 ? (
              <EmptyState
                ...
                description="Approved businesses with a Ventures package will show up here."
              />
            ) : (
              <EmptyState
                icon={SearchIcon}
